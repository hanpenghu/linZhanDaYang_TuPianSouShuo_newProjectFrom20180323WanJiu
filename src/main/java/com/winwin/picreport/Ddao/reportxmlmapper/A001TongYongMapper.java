...
//   @Select({"select top 80 olefield from up_def  where olefield like '%SamplesSys%'  and prd_no =#{prdNo} and price_id=#{priceId} group by olefield"})
  ...
  List<DaoChu> getPrdtSamp001(@Param("idList") List<String>idList);
...
//    List<String> getIdUseConfirmTime(@Param("startConfirmTime") String startConfirmTime,@Param("endConfirmTime")String endConfirmTime);
    ...
    @Select({"select a.olefield from (select top 80 olefield from up_def where prd_no=#{prdNo}  and price_id=#{priceId} order by s_dd DESC) a group by a.olefield"})
    List<String> selectSaleOlefield(@Param("prdNo")String prdNo,@Param("priceId")String priceId);

    List<String> selectTop80OlefiledAndNoIn(@Param("prdNo")String prdNo
                                            ,@Param("olefield")String olefield,
                                            @Param("priceId")String priceId,
    ...
    @Select({"select count(*) from up_def where prd_no=#{prdNo} and price_id=#{salPriceId} and cur_id=#{curId} and bil_type=#{bilType} and s_dd<=#{osDd} and #{up}<up"})
    int countPriceOfSmallThenUpdef(@Param("osDd") String osDd, @Param("prdNo")String prdNo, @Param("salPriceId")String salPriceId, @Param("up")BigDecimal up,@Param("curId") String curId,@Param("bilType") String bilType);
...
//    int upDateSampSendNull(@Param("id")String id);
    ...
    @Update({"update prdt_samp set ${fuck}=null where id=#{id}"})
    int upDateWhuColumnNull(@Param("fuck") String fuck,@Param("id")String id);



    int testMybatisUpdate(@Param("miniOrderAmt") BigDecimal b,@Param("stopusedate") Date d,@Param("stopusedateBiJiao") Date d1);
